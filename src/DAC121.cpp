...
void DAC121_initRegister(uint8_t i2cAddr, uint8_t mode, uint16_t data) {
	...
	I2C_16Bit_writeToModule(i2cAddr, maskedData | maskedMode);
}

void DAC121_setPowerDownMode(uint8_t i2cAddr, uint8_t mode) {
	uint16_t registerVal = I2C_16Bit_readFromModule(i2cAddr);

	registerVal = I2C_16Bit_setBinary(registerVal, PDM_BIT_1_POS, GET_BIT_VALUE(mode, 1));
	registerVal = I2C_16Bit_setBinary(registerVal, PDM_BIT_0_POS, GET_BIT_VALUE(mode, 0));

	I2C_16Bit_writeToModule(i2cAddr, registerVal);
}
...
void DAC121_setData(uint8_t i2cAddr, uint16_t data) {
	...
	I2C_16Bit_writeToModule(i2cAddr, maskedData | registerVal);
}

uint16_t DAC121_getData(uint8_t i2cAddr) {
	return I2C_16Bit_readFromModule(i2cAddr) & DATA_MASK;
}