...
void DAC121_setPowerDownMode(uint8_t i2cAddr, uint8_t mode) {
	...
	registerVal = I2C_setBinary(registerVal, PDM_BIT_1_POS, GET_BIT_VALUE(mode, 1));
	registerVal = I2C_setBinary(registerVal, PDM_BIT_0_POS, GET_BIT_VALUE(mode, 0));

	I2C_writeToModule(i2cAddr, registerVal);
}
...
void DAC121_setData(uint8_t i2cAddr, uint16_t data) {
	...
	I2C_writeToModule(i2cAddr, maskedData | registerVal);
}

uint16_t DAC121_getData(uint8_t i2cAddr) {
	return I2C_readFromModule(i2cAddr) & DATA_MASK;
}